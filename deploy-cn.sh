#!/bin/bash
# ChessDB 国内 VPS 一键部署脚本
# 适用于 Ubuntu 22.04/24.04 (雨云/腾讯云/阿里云)
# 使用方法: bash deploy-cn.sh

set -e

echo "============================================"
echo "  ChessDB 国内 VPS 一键部署脚本"
echo "============================================"
...
if [ -d "$REPO_DIR" ]; then
    echo ""
    echo "[3/7] 项目目录已存在，拉取最新代码..."
    cd "$REPO_DIR"
    git pull || true
else
    echo ""
    echo "[3/7] 克隆项目..."
    read -p "请输入 GitHub/Gitee 仓库地址: " REPO_URL
...
# 初始化数据库
echo "初始化数据库..."
sudo docker compose exec -T backend flask init-db 2>/dev/null || \
    echo "数据库可能已初始化，跳过..."

# 灌入种子数据
echo "灌入种子数据 (棋手、开局、示例棋谱)..."
sudo docker compose exec -T backend flask seed-data 2>/dev/null || \
    echo "种子数据可能已存在，跳过..."
...
# 检查后端
if curl -s http://localhost:5000/api/analysis/engines > /dev/null 2>&1; then
    echo "后端服务: 正常"
else
    echo "警告: 后端服务可能未就绪，请检查日志: sudo docker compose logs backend"
fi
...
if curl -s http://localhost:80 > /dev/null 2>&1; then
    ...
    echo "警告: 前端服务可能未就绪，请检查日志: sudo docker compose logs frontend"
fi
...
echo "  后端 API: http://${PUBLIC_IP}:5000/api/"
echo ""
echo "  创建管理员账户:"
